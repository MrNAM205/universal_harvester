...
def validate_cookies():
    ...
    if missing:
        ...
        print(f"✔ Required cookies present ({', '.join(REQUIRED_COOKIES)}).")

    now = time.time()
    expired = [c.get("name") for c in cookies if c.get("expires", -1) != -1 and c.get("expires", 0) < now]
    
    if expired:
        print(f"⚠ WARNING: {len(expired)} cookies have expired: {', '.join(expired[:5])}...")
    else:
        print("✔ No expired cookies detected.")

    print("================================")
    if not missing and not expired and len(cookies) > 10:
        print("✔ Cookies look GOOD. Authentication should succeed.")
    else:
        print("❌ Cookies look BAD. Please re-run export_cookies.py.")
